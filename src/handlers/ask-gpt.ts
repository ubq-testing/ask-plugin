...
export async function askQuestion(context: Context, question: string) {
    const { logger, payload: { issue: currentIssue } } = context;
    ...
    } = await getSpecReferencedContext(context, org, issueSpecOrPullBody);
    ...
    const { linkedPulls, specAndBodies } = await getSpecBodiesAndLinkedPulls(context, repository_url, currentIssue.number, issueSpecOrPullBody, specReferencedIssueBody, specReferencedIssueKey, linkedIssues);
...
async function getAllStreamlinedComments(
    ...
    if (streamlinedSpecReferencedIssueComments &&
        Object.keys(streamlinedSpecReferencedIssueComments).length > 0) {
...
async function getSpecReferencedContext(
    context: Context,
    org: string,
    issueSpecOrPullBody: string | null,
) {
...
function createKey(issueUrl: string, issue?: number) {
    ...
    let issueOrg = splitUrl?.slice(-3, -2).join("/");
...
export async function askGPT(context: Context, formattedChat: string) {
    const { logger, config: { openAi_apiKey } } = context;
